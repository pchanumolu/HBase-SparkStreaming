...
/**
 * Created by chanumolu on 6/2/15.
 */

object HBaseSensorStream {
  final val tableName = "/user/chanumolu/sensor"
  ...
  def main(args: Array[String]): Unit = {
    ...
    jobConfig.set("mapreduce.output.fileoutputformat.outputdir", "/user/chanumolu/out")
    ...
    val sensorDStream = ssc.textFileStream("/user/chanumolu/stream").map(Sensor.parseSensor)
    sensorDStream.print()
